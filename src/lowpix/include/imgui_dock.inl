	...
	struct DockContext
	{
		...
		void splits()
		{
			...
			for (int i = 0; i < m_docks.size(); ++i)
			{
				...
				{
					SetMouseCursor(dock.isHorizontal() ? ImGuiMouseCursor_ResizeEW : ImGuiMouseCursor_ResizeNS);
					if (IsMouseClicked(0))
					{
		...
		void save(FILE* f)
		{
			...
			for (int i = 0; i < m_docks.size(); ++i)
			{
				...
				fprintf(f, "active = %d,\n", (int32_t)dock.active);
				fprintf(f, "opened = %d,\n", (int32_t)dock.opened);
				fprintf(f, "prev = %d,\n", (int32_t)getDockIndex(dock.prev_tab));
				fprintf(f, "next = %d,\n", (int32_t)getDockIndex(dock.next_tab));
				fprintf(f, "child0 = %d,\n", (int32_t)getDockIndex(dock.children[0]));
				fprintf(f, "child1 = %d,\n", (int32_t)getDockIndex(dock.children[1]));
				fprintf(f, "parent = %d\n", (int32_t)getDockIndex(dock.parent));
